...
class Board(object):
    ...
    def __init__(self, size, piece_rows, resBoard=None, res=False):
        if res:
            ...
            for row in range(1, self.size + 1):
                for col in range(1, self.getColNum(row) + 1):
                    self.board_status[(row, col)] = resBoard[(row, col)]
    ...
    def getColNum(self, row):   # (maxRow, maxCol), (1, 1), (2, 2), ..., (10, 10), (11, 9), (12, 8), ..., (19, 1)
        if row in range(1, self.size + 1):
            return row
        else:
            return self.size * 2 - row

    def isEmptyPosition(self, pos):  # check if the position is empty
        return self.board_status[pos] == 0

    def leftPosition(self, pos):    # if the left position exist, return (row, col - 1), else return None
        row = pos[0]
        col = pos[1]
        if (row, col - 1) in list(self.board_status.keys()):
            return (row, col - 1)

    def rightPosition(self, pos):   # if the right position exist, return (row, col + 1), else return None
        row = pos[0]
        col = pos[1]
        if (row, col + 1) in list(self.board_status.keys()):
            return (row, col + 1)

    def upLeftPosition(self, pos):   # return upleft
        row = pos[0]
        col = pos[1]
        if row <= self.size and (row - 1, col - 1) in list(self.board_status.keys()):  # upper half plane
            return (row - 1, col - 1)
        if row > self.size and (row - 1, col) in list(self.board_status.keys()):  # down half plane
            return (row - 1, col)

    def upRightPosition(self, pos):    # return upright
        row = pos[0]
        col = pos[1]
        if row <= self.size and (row - 1, col) in list(self.board_status.keys()):  # upper half plane
            return (row - 1, col)
        if row > self.size and (row - 1, col + 1) in list(self.board_status.keys()):  # down half plane
            return (row - 1, col + 1)

    def downLeftPosition(self, pos):  # return downleft
        row = pos[0]
        col = pos[1]
        if row < self.size and (row + 1, col) in list(self.board_status.keys()):   # upper half plane
            return (row + 1, col)
    ...
    def downRightPosition(self, pos):  # return downright
        row = pos[0]
        col = pos[1]
        if row < self.size and (row + 1, col + 1) in list(self.board_status.keys()):  # upper half plane
            return (row + 1, col + 1)
    ...
    def adjacentPositions(self, pos):  # return adjacent positions
        result = []
            ...
            for row in range(1, self.piece_rows + 1):
                for col in range(1, self.getColNum(row) + 1):
                    if row == 2 and self.board_status[(row, col)] == 3:
                        ...
                        # if any of the marbles of player 2 are still in its own triangle and iter > 100, he loses.
                        return True
                    else:
                        return False
            ...
            for row in range(self.size * 2 - self.piece_rows, self.size * 2):
                for col in range(1, self.getColNum(row) + 1):
                    ...
                    if row == self.size * 2 - self.piece_rows + 1 and col == 2 and self.board_status[(row, col)] == 4:
                        ...
                        # if any of the marbles of player 1 are still in its own triangle and iter > 100, he loses.
                        return True
                    else:
                        return False
    ...
    def isEnd(self, iter):  # check if any of the player is
